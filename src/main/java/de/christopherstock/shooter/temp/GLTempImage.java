    ...
    class GLTempImage {
         ...
         * @param imgName
         */
        public GLTempImage(String imgName)
        {
            BufferedImage img = loadJavaImage(imgName);
            if (makeGLImage(img,true,false)) {
                GLTempApp.msg("GLImage(String): loaded " + imgName + ", width=" + w + " height=" + h);
            }
        }
         ...
         * @param imgName
         */
        public GLTempImage(String imgName, boolean flipYaxis, boolean convertPow2)
        {
            BufferedImage img = loadJavaImage(imgName);
            if (makeGLImage(img,flipYaxis,convertPow2)) {
                GLTempApp.msg("GLImage(String,bool,bool): loaded " + imgName + ", width=" + w + " height=" + h);
            }
        }
         ...
         * @param h
         */
        public GLTempImage(byte[] bytes) {
            BufferedImage img = makeBufferedImage(bytes);
            if (makeGLImage(img,true,false)) {
                GLTempApp.msg("GLImage(byte[]): loaded image from bytes[" + bytes.length + "]");
            }
            else {
                GLTempApp.err("GLImage(byte[]): could not create Image from bytes[" + bytes.length + "]");
            }
        }
         ...
         * @param h
         */
        public GLTempImage(byte[] bytes, boolean flipYaxis, boolean convertPow2) {
            BufferedImage img = makeBufferedImage(bytes);
            if (makeGLImage(img,flipYaxis,convertPow2)) {
                GLTempApp.msg("GLImage(byte[],bool,bool): loaded image from bytes[" + bytes.length + "]");
            }
            else {
                GLTempApp.err("GLImage(byte[],bool,bool): could not create Image from bytes[" + bytes.length + "]");
            }
        }
        ...
        public BufferedImage makeBufferedImage(byte[] imageFileContents) {
            ...
            try {
                ...
                GLTempApp.err("GLImage.makeBufferedImage(): " + ioe);
            }
            return bi;
        }
         ...
         * @param h
         */
        public GLTempImage(ByteBuffer gl_pixels, int w, int h) {
            if (gl_pixels != null) {
                   pixelBuffer = gl_pixels;
                pixels = null;
                h = h;
                w = w;
            }
        ...
        public boolean makeGLImage(BufferedImage tmpi, boolean flipYaxis, boolean convertToPow2) {
            if (tmpi != null) {
                ...
                textureW = GLTempApp.getPowerOfTwoBiggerThan(w); // the texture size big enough to hold this image
                textureH = GLTempApp.getPowerOfTwoBiggerThan(h); // the texture size big enough to hold this image
        ...
        public BufferedImage loadJavaImage(String imgName) {
            ...
            try {
                tmpi = ImageIO.read(GLTempApp.getInputStream(imgName));
            }
            catch (Exception e) {
                GLTempApp.err("GLImage.loadJavaImage() exception: FAILED TO LOAD IMAGE " + e);
            }
            return tmpi;
        }
        ...
        public static int[] getImagePixels(Image image)
        {
            ...
            if (image != null) {
                ...
                try {
                    ...
                    GLTempApp.err("Pixel Grabbing interrupted!");
                    return null;
                }
            }
            return pixelsARGB;
        ...
        public static ByteBuffer convertImagePixelsARGB(int[] jpixels, int imgw, int imgh, boolean flipVertically) {
            ...
            ByteBuffer bb = GLTempApp.allocBytes(jpixels.length * 4);  // 4 bytes per pixel
        ...
        public static void savePixelsToPNG(int[] pixels, int width, int height, String imageFilename, boolean flipY) {
            if (pixels != null && imageFilename != null) {
                if (flipY) {
                    ...
                    pixels = GLTempImage.flipPixels(pixels, width, height);
                }
                try {
                    ...
                    GLTempApp.err("GLImage.savePixelsToPNG(" +imageFilename+ "): exception " + e);
                }
            }
        }
        ...
        public static BufferedImage convertToPowerOf2(BufferedImage bsrc) {
            ...
            int newW = GLTempApp.getPowerOfTwoBiggerThan(bsrc.getWidth());
            int newH = GLTempApp.getPowerOfTwoBiggerThan(bsrc.getHeight());
